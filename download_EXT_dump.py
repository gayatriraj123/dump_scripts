...
# ====== Load Credentials ======
EXT_SSH_KEY_PATH = os.getenv("EXT_SSH_KEY_PATH")
EXT_SSH_USER = os.getenv("EXT_SSH_USER")
EXT_SSH_HOST = os.getenv("EXT_SSH_HOST")
...
def create_ssh_client():
    key = paramiko.RSAKey.from_private_key_file(EXT_SSH_KEY_PATH)
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(EXT_SSH_HOST, username=EXT_SSH_USER, pkey=key)
    return client
...
def download_db_dump():
    ...
    dump_cmd = f"mysqldump -u {EXT_DB_USER} -p'{EXT_DB_PASSWORD}' -h {EXT_DB_HOST} -P {EXT_DB_PORT} {EXT_DB_NAME} > {DUMP_FILE_REMOTE}"
